...
export function activate(context: vscode.ExtensionContext) {
    let directorySizesBefore: string[] = [];
    ...
    const tinyfy = vscode.commands.registerCommand('static.tinyfy', async () => {
        ...
        const configKey:string = await config.get('tinyKey') || '';
        (tinify as any).key = configKey;
        tinify.default.key = configKey;
        
        if(((tinify as any).key && (tinify as any).key.length > 1) || (tinify.default.key && tinify.default.key.length > 1)){
    ...
    const setTinyApiKey = vscode.commands.registerCommand('static.setTinyKey', async () => {
        ...
        const currentKey = vscode.workspace.getConfiguration('static').get<string>('tinyKey') || '';
        ...
        if (input !== undefined) {
            ...
            vscode.window.showInformationMessage(`TinyPNG API key is updated to: ${vscode.workspace.getConfiguration('static').get<string>('tinyKey')}`);
        }
    });

    function createZipArchive(directoryPath: string) {
            ...
            if (workspaceFolders) {
                workspaceFolders.forEach(folder => {
                    ...
                    subdirectories = fs.readdirSync(folder.uri.fsPath).filter(subdir => {
                        ...
                        if(check){
                            const fileSize:number = getDirectorySize(path.join(folder.uri.fsPath, subdir));
                            const fileSizeString: string = fileSize > 1000 ? (fileSize / 1000).toFixed(2) + ' MB' : fileSize + ' kB';
